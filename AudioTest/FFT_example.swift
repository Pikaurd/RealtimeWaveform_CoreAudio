...
class FFT_example {
    ...
    func calculate(_ _values: [Double], fps: Double) {
        ...
        valuesAsComplex = values.withUnsafeBytes { (x: UnsafeRawBufferPointer) -> UnsafeMutablePointer<DSPDoubleComplex>? in
            guard let rawPointer = x.baseAddress
                else { return nil }
            let unsafePointer: UnsafePointer<DSPDoubleComplex> = rawPointer.bindMemory(to: DSPDoubleComplex.self, capacity: values.count)
